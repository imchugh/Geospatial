...
def plot_topo(file_path, marker_lat, marker_lon):

# Get the data
#target = '/mnt/fluxdata/data_Digby/Spatial_data/CLIP.asc'
...
# latitude as base coordinate rather than southernmost
    ...
    hdr = [linecache.getline(file_path, i) for i in range(1,7)]
    ...
    southernmost_lat = ly 
    northernmost_lat = southernmost_lat + cell * rows
    westernmost_lon = lx
    easternmost_lon = westernmost_lon + cell * cols
    centre_lat = (southernmost_lat + northernmost_lat) / 2
    centre_lon = (easternmost_lon + westernmost_lon) / 2
    ...
    new_cmap = _truncate_colormap(cmap, 0.75, 0.2)
    ...
    contour = map.contour(xx, yy, data, levels = 80, colors = 'k')
    ...
    point_x, point_y = map(marker_lon, marker_lat)
    map.plot(point_x, point_y, marker = 's', color = 'black', markersize = 8)
    plt.text(point_x + 200, point_y - 250, 'Tower', fontsize = 18)
    ...
    scale_bar_lat_loc = southernmost_lat + abs(northernmost_lat - southernmost_lat) * 0.9
    map.drawmapscale(scale_bar_lon_loc, 
                     scale_bar_lat_loc, 
                     marker_lon, marker_lat, 2000, 
